...
def knn(x: torch.Tensor, y: torch.Tensor, k: int,
    ...
    """
    For each element in y tensor, find k nearest neighbor in x tensor
    Args:
        x: Node feature matrix :math:`\mathbf{X} \in \mathbb{R}^{N \times F}
        y: Node feature matrix :math:`\mathbf{X} \in \mathbb{R}^{M \times F}
        k: number of neighbor: 
        batch_x: :math:`\mathbf{b} \in {\{ 0, \ldots, B-1\}}^N
        batch_y: :math:`\mathbf{b} \in {\{ 0, \ldots, B-1\}}^N
        cosine: optional: cosine distance vs euclicean distance
    Return:
        
    """ 
    # if dim of x ==1, change to column vector
    x = x.view(-1, 1) if x.dim() == 1 else x
    y = y.view(-1, 1) if y.dim() == 1 else y
    if x.is_cuda:
        if batch_x is not None:
            assert x.size(0) == batch_x.numel() ## if number of element in tensor x is the same as batch_x
